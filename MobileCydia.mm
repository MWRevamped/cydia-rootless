...
@synchronized (database_) {
    ...
    if (strcmp(name, "cydia") != 0) {
        ...
        if (NSArray *files = [self files])
            for (NSString *file in files)
                if (!cydia && [file isEqualToString:@"/var/jb/Applications/Cydia.app"])
                    ...
                    dsstore = true;
                else if (!_preboot && [file isEqualToString:@"/private/preboot"])
                    _preboot = true;
        ...
        if (_preboot)
            [warnings addObject:[NSString stringWithFormat:UCLocalize("FILES_INSTALLED_TO"), @"/private/preboot"]];
    }
...
@synchronized (self) {
    ...
    int chk(creat("/var/jb/tmp/cydia.chk", 0644));
    if (chk != -1)
        close(chk);
...
- (void) configure {
    NSString *dpkg = [NSString stringWithFormat:@"/var/jb/usr/libexec/cydo --configure -a --status-fd %u", statusfd_];
    _trace();
...
- (void) _setupMail:(MFMailComposeViewController *)controller {
    [controller addAttachmentData:[NSData dataWithContentsOfFile:@"/var/jb/tmp/cydia.log"] mimeType:@"text/plain" fileName:@"cydia.log"];

    system("/var/jb/usr/bin/dpkg -l >/var/jb/tmp/dpkgl.log");
    [controller addAttachmentData:[NSData dataWithContentsOfFile:@"/var/jb/tmp/dpkgl.log"] mimeType:@"text/plain" fileName:@"dpkgl.log"];
}
...
- (void) _updateIgnored {
    ...
    FILE *dpkg(popen("/var/jb/usr/libexec/cydia/cydo --set-selections", "w"));
...
- (void) reloadSpringBoard {
    if (kCFCoreFoundationVersionNumber >= 700) // XXX: iOS 6.x
        system("/var/jb/usr/libexec/cydia/cydo /var/jb/bin/launchctl stop com.apple.backboardd");
    else
        system("/var/jb/usr/libexec/cydia/cydo /var/jb/bin/launchctl stop com.apple.SpringBoard");
    sleep(15);
    system("/var/jb/usr/bin/killall backboardd SpringBoard");
}
...
- (void) applicationDidFinishLaunching:(id)unused {
    ...
    if (access("/var/jb/.cydia_no_stash", F_OK) == 0);
    else {
    ...
    Stash_("/var/jb/Applications");
    Stash_("/var/jb/Library/Ringtones");
    Stash_("/var/jb/Library/Wallpaper");
    Stash_("/var/jb/usr/include");
    Stash_("/var/jb/usr/share");
...
int main(int argc, char *argv[]) {
    ...
    int fd(open("/var/jb/tmp/cydia.log", O_WRONLY | O_APPEND | O_CREAT, 0644));
    dup2(fd, 2);
    close(fd);
    ...
    _trace();
    mkdir("/var/jb/var/mobile/Library/Cydia", 0755);
    MetaFile_.Open("/var/jb/var/mobile/Library/Cydia/metadata.cb0");
    _trace();
    ...
    if (access("/var/jb/User", F_OK) != 0 || version != 6) {
        _trace();
        system("/var/jb/usr/libexec/cydia/cydo /var/jb/usr/libexec/cydia/firmware.sh");
        _trace();
    }

    if (access("/var/jb/tmp/cydia.chk", F_OK) == 0) {
    ...
    system([[NSString stringWithFormat:@"/var/jb/usr/libexec/cydia/cydo /var/jb/bin/ln -sf %@ /var/jb/etc/apt/sources.list.d/cydia.list", Cache("sources.list")] UTF8String]);
    ...
    _config->Set("Dir::Bin::Methods", "/var/jb/Applications/Cydia.app");
