...
if [[ ${cpu} == arm || ${cpu} == arm64 ]]; then
    ...
    data=/var/jb/Library/Cydia/dpkg
    model=hw.model
    arch=cydia
fi
...
if [[ ${cpu} == arm || ${cpu} == arm64 ]]; then
    if [[ ! -h /var/jb/User && -d /var/jb/User ]]; then
        cp -afT /var/jb/User /var/jb/var/mobile
    fi && rm -rf /var/jb/User && ln -s "/var/jb/var/mobile" /var/jb/User
